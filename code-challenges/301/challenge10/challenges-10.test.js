...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let fives = input.map((row) => {
    return row.filter(nums => typeof nums === 'number' && nums % 5 === 0).map(nums => Math.pow(2, nums));
  });
  return fives;
};
...
let findMaleAndFemale = (data) => {
  // let gender = data.reduce((acc, val)=>{
  //   if(data.filter(Obj.gender === 'male' || Obj.gender ==='female'));
  //   return data.map(x=>x.name).join(' and ');
  // },0);

  let gender = data.filter(obj => obj.gender === 'male' || obj.gender === 'female').map(obj => obj.name).join(' and ');
  return gender;
};
...
let findShortest = (data) => {
  // let smallest = data.map(obj => obj.height).reduce((acc, val)=>)
  let smallest = data.reduce((acc, val) => {
