...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    let amounts = element.slice(element.indexOf(' ') + 1);
    let units = amounts.slice(amounts.indexOf(' ') + 1)
    result.push(units);
  });
  return result;
}
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredients)=>{
    let sameAs3 = ingredients.split(' ').slice(2).join(' ');
    result.push(sameAs3);
  })
  return result;
}
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(element => {
    let actionVerb = element.split(' ')[0];
...
const extractVowels = (str) => {
  let vowels = /[aeiou]/g;
  let consonant = str.split('');
  let vowelArray = [];
  let consonantArray = [];
  for (let i=consonant.length-1; i>=0; i--){
    if (vowels.test(consonant[i])){
      vowelArray.push(consonant.splice(i, 0)[0])
    }
  }
  let results = [consonant.join(''), vowelArray.sort().join('')];
  return results;
};
