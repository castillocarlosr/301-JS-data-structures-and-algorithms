...
const filterStringsWithVowels = (arr) => {
  let regex = /[aeiou]/;
  let filterVowels = arr.filter((val,idx) =>{
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notIncluded = arr.filter((val)=>{
    return (!forbiddenValues.includes(val))
  });
  return notIncluded;
};
//found includes method on https://stackoverflow.com/a/1473742
//checked MDN for more on includes() and filter()
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  let greaterStat = arr.filter((val)=>{
...
const getStatName = (arr, minBaseStat) => {
  ...
  let greaterStat = arr.filter((val)=>{
    return (val.baseStat > minBaseStat).map(val => val.stat.name)
  });
  return greaterStat;
};
//I will look at this some more before class.  If not, this is as far as I got.
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const evenOddNumericValues = (arr) => {
  //use typeof for 'numbers' map for even odd.
};
