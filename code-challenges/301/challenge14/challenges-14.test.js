...
const toTitleCase = (arr) => {
  ...
  for(let i=0; i<arr.length; i++){
    first.push(arr[i].charAt(0).toUpperCase() + arr[i].slice(1));
  }
  return first;
};
...
let biggerThanLuke = (arr) => {
  let lukeMass = 77;
  let mass = arr.filter(obj => obj.mass > lukeMass).map(obj => obj.name).join(' - ');
  return mass;
}
...
const detectTicTacToeWin = (board) => {
  ...
  for(var i = 0; i < board.length; i++){
    oneDboard = oneDboard.concat(board[i]);
  }
  if(oneDboard[0] === oneDboard[1] === oneDboard[2] || oneDboard[0] === oneDboard[3] === oneDboard[6] ||
    oneDboard[3] === oneDboard[4] === oneDboard[5] || oneDboard[1] === oneDboard[4] === oneDboard[7] ||
     oneDboard[6] === oneDboard[7] === oneDboard[8] || oneDboard[2] === oneDboard[5] === oneDboard[8] ||
      oneDboard[0] === oneDboard[4] === oneDboard[8] || oneDboard[2] === oneDboard[4] === oneDboard[6]){
    return true;
  } else{
    return false;
  }
};
//should work.  Checked for all rows, columns, and diagnoals.  I don't think I missed a row.  I drew it on paper.
/* ------------------------------------------------------------------------------------------------
TESTS
