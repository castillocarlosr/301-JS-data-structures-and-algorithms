...
const countNumberOfElements = (arr) => {
  //[1, 2, 3, 4, 5]  should equal 5 length
  let counts = arr.reduce((accumulator, value, idx)=>{
    accumulator++;
    return accumulator;
  });
  return counts;
...
const returnNames = (arr) => {
  //works for map too.
//let characters = starWarsData.map((val, idx)=> val.name);
  let characters = arr.reduce((accumulator, person)=>{
...
const reversedString = (arr) => {
  let strr = arr.split('');
  let reversedOrder = strr.reduce((newString, currentLeter) =>{
    return currentLeter + newString;
  },'');
...
const countNumberOfChildren = (arr) => {
  let gameOfChildren = arr.reduce((accumulator, value)=>{
    if(value.children)
      return accumulator + value.children.length;
    else //This is because some don't have a key and thus value of children.
      return accumulator;
  }, 0);
  return gameOfChildren;
};
...
const calculateAverage = (arr) => {
  let avgVal = arr.reduce((acc, value)=>{
    // return (acc + value)/arr.length;
    return (acc + value);
  })/arr.length;
  return avgVal;
};
//found help on this one from https://stackoverflow.com/a/29544442
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const countPrimeNumbers = (arr) => {
  let optimusPrime = arr.reduce((acc, value)=>{
    if(isPrime(value))
      return acc+1;
    else
      return acc;
...
// Apperiantly isPrime() is a reserved function word.
