...
const firstLetters = (arr) => {
  ...
  for(let i=0; i<arr.length; i++){
    first.push(arr[i].charAt(0));
  }
  //   let result1 = first.split();
  return first;
};
...
const standardizePhoneNumbers = (arr) => {
  let nospace = [];
  for(let i=0; i<arr.length; i++) {
    let number1 = (arr[i].replace(/-/g,'').replace(/[( )]/g,''));
    nospace.push(number1);
  }
  return nospace;
...
const unenrollBrook = (arr) => {
  ...
  let noname = 'Brook';
  for(let i=0; i<arr.length; i++){
    let finalnames = arr[i].filter(function(item){
      return item !== noname;
    })
    return finalnames;
  }
